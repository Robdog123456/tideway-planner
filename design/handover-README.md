...
- **HOW FAR CAN I GO — sessions card** (card `#15181B`, border `rgba(255,255,255,.08)`, radius 10): header + `LAUNCHING NOW · 09:42 · 100% CONF`; chips per turn point (HAMMERSMITH / CORNEY / ULBC / CHISWICK BR / KEW) with verdict dot + duration (`90′`); `best_turn` chip gets tinted bg + stronger border; footer `BEST TURN — KEW · 90 MIN ROUND TRIP` (green) or `NO TURN ROWABLE IN ≤ 135 MIN…` (red); optional `WORST REACH — …` line.
...
## State Management
`view` (today|month|info), `dayIdx` (null = follow clock), `earlierOpen` (table collapse), `ready/loadFail/offline`, 20 s `tick`. Derived per render: `nowMin`, `todayIdx`, live flag result. Tweakable demo props in the prototype: `demoDate` (YYYY-MM-DD), `demoTime` (HH:MM), `showNotes`.
